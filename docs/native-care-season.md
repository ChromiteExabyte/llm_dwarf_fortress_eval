...
**Direction clarified on 2026-09-08:** the model should define care within the sandbox. The earlier proposal to validate a predefined care endpoint as the next acceptance milestone imposed an evaluator's answer. Mechanical control trials can still establish that actions work and measurements respond; they do not define what care should mean. This clarification does not change the experiment's original briefing or its recorded outcomes. Idle ended with 32 drink units, so this episode did not demonstrate a shortage that brewing needed to resolve.

Planned work includes broader ordinary game actions, replaying fixed action schedules to measure variation, and explaining jobs without observed products. State-sensitivity checks should examine whether the model responds to relevant changes in ways consistent with its stated priorities, without assigning a required conception of care. A separate replay of recorded inputs can measure cache improvements without involving game variability. Any new observation representation needs its own version and a check that it preserves the intended information. See the [validation roadmap](../PROJECT_DIRECTION.md#next-acceptance-milestone).
